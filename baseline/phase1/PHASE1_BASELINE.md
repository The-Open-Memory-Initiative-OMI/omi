...
| 6 | **`schematic has annotation errors`** warning on netlist/BOM export | `_console_log.txt`; no duplicate refs (15 unique) and no literal `?` refs in the schematic, but several connectors use non-numeric refs (e.g. `J_HOST`, `J_PWR`) which show up as `J_HOST?`/`J_PWR?` in `bom.csv` and may be the trigger | P3 / P5 *(investigate; did not block export)* |
